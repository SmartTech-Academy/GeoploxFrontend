...
export const useDeletePropertyImage = () => {
  return useMutation({
    mutationFn: (property_image_url: string) =>
      api.post(`/dashboard/property/image/delete`, {
...
export const useRemoveFromFavorites = (queryKeyToInvalidate: string[] = ["property"]) => {
  return useMutation({
    mutationFn: (propertyId: string | undefined) =>
      api.post(`/dashboard/favorite/property-removal/${propertyId}`),
    onSuccess: (data) => {
