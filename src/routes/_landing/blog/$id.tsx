...
import { useInfiniteWpPosts, useWpPostBySlug } from '@/lib/services/wpBlog';
import { excerptFromHtml } from '@/lib/utils';
import { BLOG_BASE_URL, toAbsoluteBlogUrl } from '@/lib/wpGraphql';
          ...
          <div className="flex flex-col items-start self-stretch">
            ...
            <div className="flex flex-col gap-12">
              {postQuery.isLoading ? (
                ...
                <div className="rounded-xl border border-[#E9EBEC] bg-white p-6 text-[#060809]">Post not found.</div>
              ) : (
                <>
                  <div className="flex items-center gap-3">
                    <span className="text-[15px]/5 font-semibold tracking-[-0.12px] text-[#060809]">{category}</span>
                    {postDate ? (
                      <span className="text-[15px]/5 tracking-[-0.12px] text-[#7B828E]">{postDate}</span>
                    ) : null}
                  </div>

                  <div className="flex w-full flex-col gap-[15px]">
                    <h1 className="text-[45px] leading-[58px] font-semibold tracking-[-1.21px] text-black">{title}</h1>

                    {description ? (
                      <p className="text-[15px]/5 tracking-[-0.12px] text-[#060809]">{description}</p>
                    ) : null}
                  </div>
                  ...
                  <img
                    ...
                    className="max-h-[541px] h-auto w-full rounded-xl object-cover"
                    width={1212}
                    height={541}
                  />

                  <div className="flex flex-col items-start gap-6 px-5 lg:px-6">
                    <div className="wp-content w-full" dangerouslySetInnerHTML={{ __html: post.content ?? '' }} />
                    <div className="flex w-full flex-col items-start gap-[25px]">
                      <div className="flex items-center gap-[25px]">

                        <img
                          src={toAbsoluteBlogUrl(post.author?.node?.avatar?.url) ?? assets.blogwritter}
            ...
            {morePostsQuery.isLoading ? (
              ...
              morePosts.map((p) => {
                ...
                const img =
                  ...
                  assets.blog1;
                return (
                  <Link to="/blog/$id" params={{ id: slug }} key={p.uri} className="grow rounded-xl border border-[#E9EBEC] bg-white">
